...
$name = $image = $video_name = $rating = $servings = $difficulty_text = $maxTime = $difficultyID = "";
$name_err = $image_err = $video_name_err = $rating_err = $servings_err = $difficulty_text_err = $maxTime_err = $difficultyID_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
                 ...
                 $difficulty_text = $input_difficulty_text;
                   ...
                   if(empty($input_maxTime)){
                       ...
                       $maxTime = $input_maxTime;
                        ...
                        // Validate Max Time
                        $input_difficultyID = trim($_POST["difficultyID"]);
                        if(empty($input_difficultyID)){
                            $difficultyID_err = "Please enter difficulty ID (1 Easy, 2 Medium, 3 Hard).";     
                        } else{
                            $difficultyID = $input_difficultyID;
                        }
    ...
    if(empty($name_err) && empty($image_err) && empty($video_name_err) && empty($rating_err) && empty($servings_err) && empty($difficulty_text_err) && empty($maxTime_err)){
        ...
        $query = "INSERT INTO recipes (user_ID, name, image, video_name, rating, servings, difficulty_text, maxTime, difficultyID) VALUES 
        (:user_ID, :name, :image, :video_name, :rating, :servings, :difficulty_text, :maxTime, :difficultyID)";
        if($stmt = $conn->prepare($query)){
            ...
            $stmt->bindParam(":difficulty_text", $difficulty_text);
                ...
                <div class="col-md-12">
                    ...
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post">
                        ...
                        <div class="form-group <?php echo (!empty($difficultyID_err)) ? 'has-error' : ''; ?>">
                            ...
                            <input type="text" name="difficultyID" class="form-control" value="<?php echo $difficultyID; ?>">
