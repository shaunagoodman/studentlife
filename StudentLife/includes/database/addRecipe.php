...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (isset($_POST['submitRecipe'])) {
        ...
        $measures = [];
        $units = [];
    if(isset($_POST["metric_ingredient_name"])) {
        foreach ($_POST["metric_ingredient_name"] as $key => $name) {
            array_push($names, $name);
        }
    }
    else if($_POST["imperial_ingredient_name"]) {
        foreach ($_POST["imperial_ingredient_name"] as $key => $name) {
            array_push($names, $name);
        }
    }
    else {
    ...
    if(isset($_POST["metric_ingredient_measure"])) {
        foreach ($_POST["metric_ingredient_measure"] as $key => $measure) {
            array_push($measures, $measure);
        }
    }
    else if($_POST["imperial_ingredient_measure"]) {
        foreach ($_POST["imperial_ingredient_measure"] as $key => $measure) {
            array_push($measures, $measure);
        }
    }
    else {
    ...
    if(isset($_POST["metric_ingredient_unit"])) {
        foreach ($_POST["metric_ingredient_unit"] as $key => $unit) {
            array_push($units, $unit);
        }
    }
    else if($_POST["imperial_ingredient_unit"]) {
        foreach ($_POST["imperial_ingredient_unit"] as $key => $unit) {
            array_push($units, $unit);
        }
    }
    else {
